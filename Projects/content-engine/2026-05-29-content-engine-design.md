...
- **RESEARCH PAPERS (the deepest source, the whole point):** a paper-retrieval module using the arXiv API + Semantic Scholar API (both free, public, structured) to SEARCH by topic/keyword and pull abstracts + full text (PDF). Optionally OpenReview (peer reviews) + Papers with Code. Covers AI / anti-AI-critical / experiments / behavior literature. This makes the scraper a RESEARCH-RETRIEVAL engine, not just a transcript fetcher. Robust, production-grade (clean free APIs = ideal code target). This is the source that lets the engine "explore the depths."
