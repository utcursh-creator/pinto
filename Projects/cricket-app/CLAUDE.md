...
- **If broadcast tests fail with `no partition of relation "messages" found for row`**, the local `realtime.messages` daily partitions have run out (the maintainer only keeps a rolling window, and a long-lived stack outlives it). Nothing is wrong with the code - inserts into realtime.messages just fail. Create them:
  `do $$ declare d date; begin for d in select generate_series(current_date-2, current_date+7, interval '1 day')::date loop execute format('create table if not exists realtime.messages_%s partition of realtime.messages for values from (%L) to (%L)', to_char(d,'YYYY_MM_DD'), d, d+1); end loop; end $$;`
