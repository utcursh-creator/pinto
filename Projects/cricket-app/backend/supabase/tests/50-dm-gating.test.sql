...
select throws_ok($$ insert into public.dm_messages(thread_id, sender_id, body) values ((select id from public.dm_threads limit 1), tests.get_supabase_uid('c@m.dev'), 'intrude') $$, '42501', null, 'non-participant cannot send');
select * from finish();
rollback;
