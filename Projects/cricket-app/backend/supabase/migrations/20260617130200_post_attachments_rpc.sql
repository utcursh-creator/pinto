-- Thread image_urls + link_url through create_looking_for_post and surface them
-- in discover_posts. Drop the prior overloads first (added params / new return).
drop function public.create_looking_for_post(public.lf_mode,public.lf_flair,float,float,text,uuid,text,timestamptz,int,public.skill_level,int,text,timestamptz);
...
create function public.create_looking_for_post(
  ...
  _image_urls text[] default null, _link_url text default null
...
begin
  ...
  insert into public.looking_for_posts(author_id, team_id, mode, flair, title, description, geog, place_label, match_at, overs, skill, slots_needed, expires_at, image_urls, link_url)
  values (_uid, _team_id, _mode, _flair, _title, _description,
          ...
          coalesce(_image_urls, '{}'), _link_url)
...
revoke all on function public.create_looking_for_post(public.lf_mode,public.lf_flair,float,float,text,uuid,text,timestamptz,int,public.skill_level,int,text,timestamptz,text[],text) from public;
grant execute on function public.create_looking_for_post(public.lf_mode,public.lf_flair,float,float,text,uuid,text,timestamptz,int,public.skill_level,int,text,timestamptz,text[],text) to authenticated;
