...
declare
  ...
  _striker uuid; _non_striker uuid; _tmp uuid; _facing uuid; _out uuid;
...
begin
  ...
  for d in select * from public.deliveries where innings_id = _innings_id order by seq loop
    ...
    -- dismissal: FoW + incoming batter replacement
    if d.wicket_type is not null and d.wicket_type <> 'retired_not_out' then
      ...
      _fow := _fow || jsonb_build_object('wicket_number', _wickets, 'score_at_fall', _runs,
        'over', (_legal/6)::text || '.' || (_legal%6)::text, 'dismissed_player_id', _out);
      if d.incoming_batter_id is not null then
        if _striker = _out then _striker := d.incoming_batter_id;
        elsif _non_striker = _out then _non_striker := d.incoming_batter_id;
        end if;
      end if;
    end if;
