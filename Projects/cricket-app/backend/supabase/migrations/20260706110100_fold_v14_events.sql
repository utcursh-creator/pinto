-- fold v14 (SCOR-16 + SCOR-24): non-ball EVENT rows + optimistic-concurrency token.
--  * strike_swap events flip the current pair and touch nothing else.
--  * retirement events remove a batter between balls: retired_out/timed_out count
--    as wickets (FOW + all-out), retired_not_out does not; both close the
--    partnership and bring the incoming batter in. v13 never applied a
--    retired_not_out replacement at all - the retiring batter kept batting.
--  * output gains 'last_seq': the version token a scorer passes back to
--    record_ball so a stale device cannot append against a state it never saw.
-- LOCKSTEP RULE: compute_innings_cards + restamp_innings_strike updated with it.
...
begin
  select i.opening_striker_id, i.opening_non_striker_id, coalesce(m.balls_per_over,6),
         ...
         coalesce((m.rules->>'squad_size')::int, nullif((select count(*)::int from public.match_squad ms where ms.match_id = i.match_id and ms.team_id = i.batting_team_id), 0), 11), coalesce((m.rules->>'last_man_stands')::boolean, false)
