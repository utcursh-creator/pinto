-- HIGH (penetration review 2026-07-07): THE FOLD LOCKSTEP FIX.
-- compute_innings_state derived squad_size from the real match_squad, while
-- compute_innings_cards and restamp_innings_strike hardcoded 11. For any squad
-- that is not 11 the three folds disagreed about all_out, so they disagreed
-- about WHERE THE INNINGS ENDED. Proven with a 2-player squad: state said
-- 4 runs off 2 balls, cards said 16 off 4 - the scorecard invented 12 runs.
-- matches.potm is frozen from cards at result time and career stats bake from
-- cards, so this permanently corrupted real records through a sanctioned path.
--
-- Root fix: ONE function owns every innings-level parameter and all three folds
-- read it, so a future change cannot desynchronise them. Test 107 asserts the
-- invariant (cards runs + extras == state runs) directly.
create or replace function public._innings_fold_params(_innings_id uuid)
returns table(
  match_id uuid, batting_team uuid, bowling_team uuid,
  opening_striker uuid, opening_non_striker uuid,
  bpo int, count_nb_faced boolean, target int, max_overs int,
  squad_size int, lms boolean, all_out int, max_legal int
) language sql stable set search_path = public as $$
  select i.match_id, i.batting_team_id, i.bowling_team_id,
         i.opening_striker_id, i.opening_non_striker_id,
         coalesce(m.balls_per_over, 6),
         coalesce((m.rules->>'count_noball_as_ball_faced')::boolean, true),
         i.target,
         coalesce(i.revised_overs, i.overs_limit, m.overs_limit),
         sz.n,
         coalesce((m.rules->>'last_man_stands')::boolean, false),
         case when coalesce((m.rules->>'last_man_stands')::boolean, false)
              then sz.n else sz.n - 1 end,
         coalesce(i.revised_overs, i.overs_limit, m.overs_limit) * coalesce(m.balls_per_over, 6)
  from public.innings i
  join public.matches m on m.id = i.match_id
  cross join lateral (
    select coalesce(
      (m.rules->>'squad_size')::int,
      -- Only trust a DECLARED squad of >= 2. A squad of 0 or 1 is not a real
      -- batting side (all_out would be squad_size - 1 = 0, so the innings would
      -- be over before a ball was bowled); fall back to the 11 default. This
      -- floor matters now that cards/restamp share the derivation - previously
      -- only `state` saw it, and cards' hardcoded 11 masked the nonsense.
      (select nullif(count(*), 1)::int from public.match_squad ms
        where ms.match_id = i.match_id and ms.team_id = i.batting_team_id
        having count(*) >= 2),
      11) as n) sz
  where i.id = _innings_id;
$$;
revoke all on function public._innings_fold_params(uuid) from public;
grant execute on function public._innings_fold_params(uuid) to authenticated, anon;
...
grant execute on function public.compute_innings_cards(uuid) to anon, authenticated;
...
declare
  ...
  d record;
begin
  -- LOCKSTEP: identical parameters to the other two folds (this hardcoded 11).
