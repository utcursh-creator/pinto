-- SCOR-10: all-out used squad_size = rules.squad_size OR a hardcoded 11, so a
-- match with a real short squad never went all out (the fold waited for 10
-- wickets that can't fall). Derive squad_size from the actual match_squad count
-- of the batting side (rules.squad_size still overrides; fall back to 11 when no
-- squad rows exist, e.g. legacy/seed data). (compute_innings_state fold v12)
