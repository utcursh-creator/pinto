-- TOUR-5: standings previously listed only teams that had already played a
-- completed match, so most of a live tournament showed a partial ladder. Seed
-- the table from ALL entered teams (tournament_teams) at P0 and LEFT JOIN the
-- computed points/NRR. Everything else (NRR all-out full-quota rule, points,
-- tiebreak points -> NRR -> head-to-head -> name) is unchanged.
