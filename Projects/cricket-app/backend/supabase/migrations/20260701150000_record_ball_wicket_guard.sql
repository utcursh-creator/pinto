-- SCOR-2: reject a batter-removing wicket that has no incoming batter unless it
-- is the last wicket (all out). Prevents the dismissed batter from continuing
-- to score. Return type unchanged so create or replace (no drop needed).
