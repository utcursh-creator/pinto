...
-- no-ball were bucketed as byes/leg-byes and never charged to the bowler.
--
...
--
-- The innings TOTAL was never affected. What was wrong is the bowler's runs
-- conceded and economy (understated) and the extras breakdown, which read
-- "nb 1, b 2" for a delivery from which byes cannot be scored.
--
-- The console needs no change: its "The runs came from" question asks whether
-- the BAT was involved, which is exactly the distinction Law 21.13 turns on.
...
begin
  ...
  for d in select * from public.deliveries where innings_id = _innings_id order by seq loop
    ...
    -- Law 21.13: runs completed off a No ball, with its penalty, are NO BALL
    -- extras unless the bat struck it. Byes and leg-byes off a No ball do not
    -- exist, so bucketing them there claimed byes were scored off a delivery
    -- from which byes cannot be scored (review #2, finding 72).
    _wides := _wides + d.extra_wides;
    _pen := _pen + d.extra_penalty;
    if d.extra_no_ball_penalty > 0 then
      _nb := _nb + d.extra_no_ball_penalty + d.extra_byes + d.extra_leg_byes;
    else
      _nb := _nb + d.extra_no_ball_penalty;
      _byes := _byes + d.extra_byes; _lb := _lb + d.extra_leg_byes;
    end if;

    _dconc := d.runs_off_bat + d.extra_wides + d.extra_no_ball_penalty
              -- a No ball is debited to the bowler, and so is everything that
              -- results from it; excluding byes is correct ONLY on a legal ball
              + case when d.extra_no_ball_penalty > 0
                     then d.extra_byes + d.extra_leg_byes else 0 end;
