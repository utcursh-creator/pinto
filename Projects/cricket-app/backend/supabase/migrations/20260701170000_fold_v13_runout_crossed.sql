-- SCOR-3: run-out strike attribution. compute_innings_state derived strike after
-- a run-out purely from off-bat run parity and never read deliveries.crossed, so
-- a run-out where the batters crossed on the fatal run put the wrong batter on
-- strike. (compute_innings_state fold v13)
