-- Sub-project 4, Task 9: record_ball returns a wagon_applicable hint so the
-- client (iOS + Android) does not duplicate the "should we prompt for a shot?"
-- rule. Changing the return type requires DROP + CREATE (also makes PostgREST
-- reload the new shape). Body is unchanged except the final return.
...
begin
  ...
  -- false for wides / byes / leg-byes / no-ball-not-off-bat and for no-contact dismissals.
  delivery_id := _id;
  wagon_applicable :=
