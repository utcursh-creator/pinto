-- Tournament leaderboards: top run-scorers / wicket-takers / fielders (catches +
-- run-outs + stumpings) / boundary hitters, aggregated across the tournament's
-- COMPLETE matches via compute_innings_cards. Keyed by team_members.id (each
-- tournament player is one membership), with names. anon-readable.
...
names as (
  select tmem.id mid, coalesce(tmem.guest_name, p.display_name, 'Player') name
...
select jsonb_build_object(
  'most_runs', coalesce((select jsonb_agg(jsonb_build_object('member_id',a.mid,'name',n.name,'runs',a.runs) order by a.runs desc)
    from (select * from run_agg where runs > 0 order by runs desc limit 10) a join names n on n.mid=a.mid), '[]'::jsonb),
  'most_wickets', coalesce((select jsonb_agg(jsonb_build_object('member_id',a.mid,'name',n.name,'wickets',a.wkts) order by a.wkts desc)
    from (select * from wkt_agg where wkts > 0 order by wkts desc limit 10) a join names n on n.mid=a.mid), '[]'::jsonb),
  'most_catches', coalesce((select jsonb_agg(jsonb_build_object('member_id',a.mid,'name',n.name,'dismissals',a.dis) order by a.dis desc)
    from (select * from field_agg where dis > 0 order by dis desc limit 10) a join names n on n.mid=a.mid), '[]'::jsonb),
  'most_fours', coalesce((select jsonb_agg(jsonb_build_object('member_id',a.mid,'name',n.name,'fours',a.fours) order by a.fours desc)
    from (select * from run_agg where fours > 0 order by fours desc limit 10) a join names n on n.mid=a.mid), '[]'::jsonb),
  'most_sixes', coalesce((select jsonb_agg(jsonb_build_object('member_id',a.mid,'name',n.name,'sixes',a.sixes) order by a.sixes desc)
