...
begin
  ...
  for d in select * from public.deliveries where innings_id = _innings_id order by seq loop
    ...
    _bcur := coalesce(_bowlers -> _bkey, jsonb_build_object('bowler_id', d.bowler_id, 'legal_balls', 0, 'runs_conceded', 0));
    ...
    -- free-hit: a no-ball sets it; a legal ball consumes it; a wide leaves it unchanged (carry)
