...
-- It re-folds ONE innings with the SAME rules (strike rotation, the
-- count_noball_as_ball_faced ball-faced rule, the bowler-credited wicket set,
-- the maiden over-window, the same dismissal attribution and innings-end
-- priority) but emits per-player lines instead of innings totals. This is the
-- only correct basis for career stats: a flat view over deliveries.* would
-- mis-attribute bowled/caught/lbw/stumped (record_ball leaves
-- dismissed_player_id null for those - the fold attributes them to the
-- on-strike striker) and could disagree with the scorecard after a correction.
--
-- The two folds MUST stay in lockstep. compute_innings_state stays the public
-- live-viewer contract (its jsonb shape is byte-stable); this function is the
-- stats primitive. pgTAP test 61 cross-checks cards totals against
-- compute_innings_state on a shared fixture so they cannot silently diverge.
