-- fold v14 lockstep: compute_innings_cards learns event rows. A retirement marks
-- the batter's card line (retired_out/timed_out -> dismissed; retired_not_out ->
-- how_out label but NOT dismissed, so averages treat it as a not-out), applies
-- the incoming replacement, and never touches ball/bowler accounting. Strike
-- swaps just flip the pair.
