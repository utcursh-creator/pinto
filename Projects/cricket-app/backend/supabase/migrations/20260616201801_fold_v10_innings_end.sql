...
begin
  ...
  if _legal > 0 then _crr := round(_runs::numeric / (_legal::numeric/6), 2); else _crr := null; end if;
  ...
  if _target is not null then
    ...
    if _balls_rem > 0 then _rrr := round(_runs_req::numeric / (_balls_rem::numeric/6), 2); else _rrr := null; end if;
