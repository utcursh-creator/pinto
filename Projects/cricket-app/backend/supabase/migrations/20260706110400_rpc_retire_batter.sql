-- SCOR-16: record a retirement (hurt = retired_not_out, out = retired_out) as a
-- non-ball event row. The retiring batter must be at the crease; an incoming
-- batter is required unless this retirement ends the innings (last pair).
create or replace function public.retire_batter(
  _innings_id uuid, _retiring_batter_id uuid, _out boolean default false,
...
begin
  ...
  if _incoming_batter_id is null and coalesce((_state->>'wickets_remaining')::int, 99) >= 2 then
...
revoke all on function public.retire_batter(uuid, uuid, boolean, uuid) from public;
grant execute on function public.retire_batter(uuid, uuid, boolean, uuid) to authenticated;
