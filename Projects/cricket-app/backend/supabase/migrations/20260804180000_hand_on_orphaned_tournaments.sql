...
-- orphaned when its organizer deletes their account, and can never be advanced
-- or completed by anyone.
--
-- Full function re-stated because it is one transaction and the ordering
-- matters; the new work is the tournaments handover, placed with the captaincy
-- handover it mirrors.
...
begin
  ...
  -- MEDIUM (whole-system review #2, finding 36): an in-progress tournament is
  -- orphaned when its organizer deletes their account.
  --
  -- Every management RPC - add_tournament_team, set_tournament_team_group,
  -- generate_group_fixtures, generate_playoffs, advance_playoffs,
  -- resolve_tied_fixture - is gated on is_tournament_organizer, which is just
  -- `organizer_id = auth.uid()`. There is no transfer-organizer RPC anywhere,
  -- and the tournaments_write_organizer policy is the same condition, so no
  -- other account can rewrite organizer_id either. The deleted account is
  -- banned_until 'infinity' with its identities gone, so they cannot come back
  -- and finish it.
  --
  -- The public page keeps serving a half-finished bracket. Group games can
  -- still be scored, but the semifinals can never be generated, the final never
  -- created and champion_team_id never set. Every enrolled team is stuck in a
  -- competition that cannot end.
  --
  -- This is the SAME shape as the captaincy handover above, and it gets the
  -- same answer: refusing the deletion is not an option, so hand the tournament
  -- on. It goes to the longest-standing captain of the longest-established club
  -- enrolled in it - deterministic, and a real person with a stake in seeing it
  -- finished. A completed tournament needs no organizer, and one with no
  -- enrolled club has nobody to strand.
     ...
     set organizer_id = (
       select tm.profile_id
        ...
        where tt.tournament_id = t.id
          ...
          and tm.profile_id <> _me
        order by te.created_at, tm.created_at
        limit 1)
   where t.organizer_id = _me
