-- LOCAL-ONLY test harness. Runs on `supabase db reset` (NOT on `supabase db push` to prod).
-- Installs pgTAP + the open-source basejump test helpers.
-- Helpers are VENDORED (not installed via dbdev) because the in-database dbdev client is deprecated;
-- vendoring is deterministic and needs no network at reset time.
-- Source: https://github.com/usebasejump/supabase-test-helpers  (supabase_test_helpers--0.0.6.sql, MIT license).

create extension if not exists pgtap with schema extensions;

-- ================= basejump supabase_test_helpers v0.0.6 (vendored; psql meta-command guard line removed) =================
