...
## Positioning: PARTNER not employee (OTT lesson, 2026-06-15)
When a prospect demands a free TEST before deciding whether to work with you, that is a POSITIONING failure, not a sales-process step. It means you are being perceived as an EMPLOYEE / editor-for-hire to be auditioned, not as a PARTNER who owns an outcome (here: volumetric UGC-video fulfillment). Sanchit's OTT prospects did exactly this. The fix is not to give better free tests; it is to REPOSITION as a fulfillment partner so proof is offered on YOUR terms (a proactive before/after proof-reel built without being asked), never extracted as a free audition. This mirrors the Yosef "rent a technical partner" framing that already works for Utkarsh. Positioning is one of his three OTT packages (ICP / Offer / Positioning) and is the root cause of stalled deals, so it comes first.

## GTM: 6-whales outbound != 600-leads outbound (2026-06-15)
For a tiny curated list of high-fit named accounts (the Yosef model), do NOT run a generic psychological-template cadence (that's a volume tactic). Go bespoke + proof-led: T1 is a personalized proof artifact (for the OTT video service: the target's OWN reel re-edited via the system), not a clever hook. Keep sending lean (own inbox, simple board, NOT a cold-email SaaS like Smartlead - overkill for 6 sends, and it cheapens the touch). The cadence = angle-diverse follow-ups AFTER the proof open, mapped to real buyer psychology (for production-house leads: capacity-ceiling, headcount-pain, margin pressure, consistency-drift).
**Reusable buyer-fit axis (editing-ops service)**: the qualifier isn't the label ("influencer agency") but "owns an in-house editing line under volume pressure." Open editor/post-production JOB POSTINGS are a live heat-map of editing-capacity pain = warm doors.
**Fold content into the proof asset**: don't spin up a separate content workstream for an outbound play; the proof-reels ARE the content (one render, two uses: 1:1 send + post the before/after). Watch Utkarsh's recurring urge to add a content engine to everything.
**Accountability over a machine**: given his session-long GTM-avoidance, name it directly before building outreach infra - "are you going to send these or design a machine you won't push?" Non-anxious but real.
